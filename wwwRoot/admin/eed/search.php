...
$dbConn = connectNYRDb();
$items = array();
$res = mysqli_query($dbConn, "SELECT name FROM Dicts WHERE (term LIKE '%" . trim(strip_tags($_GET['term'])) . "%') ORDER BY term ASC");
while($row = mysqli_fetch_array($res)) {
	$items[] = html_entity_decode($row['term'], ENT_QUOTES, "UTF-8");
}
//echo $items;
// Close DB Connection
mysqli_close($dbConn);
