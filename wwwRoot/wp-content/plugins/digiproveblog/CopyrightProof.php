...
// in the development of this plug-in
	...
	$log = new DPLog();  
	$log->lwrite("VERSION " . DPRV_VERSION . " CopyrightProof.php starts");

	global $DPRV_SSL, $dprv_licenseIds, $dprv_licenseTypes, $dprv_licenseCaptions, $dprv_licenseAbstracts, $dprv_licenseURLs, $dprv_post_id, $dprv_mime_types, $dprv_blog_host, $dprv_wp_host, $dprv_last_error;
	$DPRV_SSL = get_option('dprv_use_ssl');
	if ($DPRV_SSL == false)
	{
		$DPRV_SSL = "No";
	}
	...
	add_action('admin_enqueue_scripts', 'dprv_edit_enqueue_scripts');
    ...
    function return_filters_for( $hook = '' )
{
    global $wp_filter;
	...
	function dprv_activate()
	{
		...
		add_option('dprv_enrolled', 'No');                  // Is User registered with Digiprove?  
		add_option('dprv_user_id', '');
		add_option('dprv_api_key', '');
		add_option('dprv_use_ssl', 'No');
	...
	function create_dprv_post_table()
	{
		...
		if ($like_dprv_posts !== false && $like_dprv_posts !== $dprv_posts)
		{
			...
			$message = "Failure to create table " . $dprv_posts . "!=(" . $like_dprv_posts . ") with error " .  $dprv_db_error;
			dprv_record_event($message);
		}
		else
		{
...
// OLD WAY TO MANAGE NOTICES  ///////////////////////////////
global $dprv_msgs, $dprv_levels;
$dprv_msgs=array();
$dprv_levels=array();
function dprv_make_notice_div()
{
	$log = new DPLog();
	global $dprv_msgs, $dprv_levels;
...
function dprv_notify($dprv_notice, $dprv_notice_level)
{
	$log = new DPLog();
	$log->lwrite("dprv_notify( " . $dprv_notice . ", " . $dprv_notice_level . ")");
	global $dprv_msgs, $dprv_levels;
	array_push($dprv_msgs, $dprv_notice);
	array_push($dprv_levels, $dprv_notice_level);
}
add_action( 'admin_notices', 'dprv_make_notice_div' );


// NEW WAY TO MANAGE NOTICES  (Uses Option fields for persistence)
function dprv_print_notices()
{
	$log = new DPLog();
	$dprv_msgs = get_option('dprv_pending_messages');
	...
	// clear arrays now
	update_option('dprv_pending_messages', array());
	update_option('dprv_pending_levels', array());
}
function dprv_add_notice($dprv_notice_text, $dprv_notice_level)
{
	$log = new DPLog();
	$log->lwrite("dprv_add_notice(" . $dprv_notice_text . ", " . $dprv_notice_level .")");
	$dprv_msgs = get_option('dprv_pending_messages');
	if (in_array($dprv_notice_text, $dprv_msgs)) {return;}
	array_push($dprv_msgs, $dprv_notice_text);
	$dprv_levels = get_option('dprv_pending_levels');
	array_push($dprv_levels, $dprv_notice_level);
	update_option('dprv_pending_messages', $dprv_msgs);
	update_option('dprv_pending_levels', $dprv_levels);
}
add_action( 'admin_notices', 'dprv_print_notices' );
...
*/
		...
		//if ($script_name !== "options-general" || empty($_POST['dprv_cp_action']))  // don't display any warnings on settings page unless it's not Postback'
		if ($script_name !== "options-general" && $script_name !== "admin-ajax")   // limit the frequency of nagging warnings
		{
			if (get_option('dprv_enrolled') != "Yes")
			{
				$dprv_configure_warning = "<strong>".__("Copyright Proof is almost ready.", "dprv_cp")."</strong> <span style=\"color:red\">".sprintf(__('You should %1$s Configure Copyright Proof and Register %2$s to get it working', 'dprv_cp'), '<a href="options-general.php?page=copyright-proof-settings"><b>', '</b></a>') . "</span>";
				dprv_add_notice($dprv_configure_warning, "warning");
			}
			else
			{
				if (trim(get_option('dprv_api_key')) == '')
				{	
					$dprv_api_key_warning = "<strong>".__("Copyright Proof needs configuration.", "dprv_cp")."</strong> ".sprintf(__('Please %s obtain a new api key %s to get it working', 'dprv_cp'), '<a href="options-general.php?page=copyright-proof-settings"><b>', '</b></a>');
					dprv_add_notice($dprv_api_key_warning, "warning");
				}
			}
		}
		...
		*/
		//$script_name = dprv_get_script_name();
		if ($dprv_pending_message != '' && ($script_name == 'admin' || $script_name == 'options-general' || $script_name == 'post' ))
		{	
			dprv_notify($dprv_pending_message, "info");
			update_option('dprv_pending_message', '');
		}
	}
