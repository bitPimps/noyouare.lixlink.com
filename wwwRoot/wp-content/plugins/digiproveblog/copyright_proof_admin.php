...
// get_option('dprv_api_key') - If empty message "Copyright Proof needs configuration"
...
function dprv_admin_head()	// runs between <HEAD> tags of admin settings page - include js file
{
	...
	$log = new DPLog();  
	$log->lwrite("dprv_admin_head begins");
...
function dprv_admin_footer($value)
{
	global $revision_id;
	$log = new DPLog();
	$log->lwrite("dprv_admin_footer begins");
	$script_name = dprv_get_script_name();
...
{		
	...
	$log = new DPLog();
	if (!empty($_POST['dprv_cp_action']))
	{
		$log->lwrite("dprv_settings starting (Postback)");
	}
	else
	{
	...
	if ($dprv_user_id != "" && $dprv_api_key != "" && $dprv_enrolled == "Yes")
	{
		$error_message = "";
		$dprv_credentials = array("user_id" => $dprv_user_id, "api_key" => $dprv_api_key, "domain_name" => $dprv_blog_host);
		$dprv_sync_response = Digiprove::get_user($error_message, $dprv_credentials, false, $user_agent);
		$log->lwrite("response from get_user: " . $dprv_sync_response);
	...
	$dprv_pw_confirm = $dprv_password;						// Used to populate pw confirm box on refresh
	$dprv_use_ssl = get_option('dprv_use_ssl');
	if ($dprv_use_ssl == false)
	{
		$dprv_use_ssl = 'No';
	}
	...
	if (!empty($_POST['dprv_cp_action']))		// Is POSTBACK, do necessary validation and take action (will not exist at all unless this is Postback)
	{
		...
		switch ($dprv_action)
		{
			...
			default:
			{
				...
				if ($result_message == "")
				{
					...
					$dprv_use_ssl = 'No';
					if (isset($_POST['dprv_use_ssl']) && $_POST['dprv_use_ssl'] == 'on')
					{
						$dprv_use_ssl = 'Yes';
					}
					update_option('dprv_use_ssl', $dprv_use_ssl);
					global $DPRV_SSL;
					$DPRV_SSL = $dprv_use_ssl;
					...
					//$dprv_privacy_accepted = false;
					//if (isset($_POST['dprv_privacy']) && $_POST['dprv_privacy'] == "on")
					//{
					//	$dprv_privacy_accepted = true;
					//}
		...
		// printing directly this notice as dprv_notify not (yet) stable going across files
		print('<div id="message" class="notice notice-' . $message_level . '"><p>' . $message . '&nbsp;&nbsp;' . $result_message . '</p></div>');
	}
	...
	*/
	...
	if ($dprv_register_option == "Yes")
	{
		...
		//$dprv_display_privacy_row = $dprv_display_register_row;
	}
	/*}*/
	if ($dprv_register_option == "No")
	{
		...
		$dprv_display_contact_row = 'style="display:none"';
		//$dprv_display_privacy_row = 'style="display:none"';
		$dprv_display_password_rows = ' style="display:none"';
	}
	...
	$dprv_use_ssl_checked = '';
	if ($dprv_use_ssl == 'Yes')
	{
		$dprv_use_ssl_checked = ' checked="checked"';
	}
	...
	//$dprv_privacy_checked = '';
	//if ($dprv_privacy_accepted == true)
	//{
	//	$dprv_privacy_checked = ' checked="checked"';
	//}
	...
	// Default Values
	// TODO - better messages explaining exactly what to do
	...
	$sub_enabled_title = ' title="This option is available to subscribers only - consider upgrading"';
	...
	$prem_enabled_title = ' title="This option is available to users at Professional or higher subscription plan"';
   ...
   // global $dprv_marker;
									...
									<tbody>
											...
											<td>
												<table style="font-size:14px;border-collapse:separate;width:796px">
													<tr>
														...
														<td id="HistoryTab" style="height:30px; width:100px; border:1px solid #666666; border-top-right-radius:5px; background-color:#AAAAEE; text-align:center; cursor:pointer' . $dprv_tabs_enabled . '" onclick="dprv_DisplayHistory()"' . $dprv_tab_title . '><em>' . __('History', 'dprv_cp') . '</em></td>
										...
										<tr id="BasicPart2">
											...
											if ($dprv_subscription_type != '')
											{
												...
												print('		&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:dprv_SyncUser();" title="' . __('Refresh Subscription Info', 'dprv_cp') . '" style="vertical-align:-3px;width:16px;height:16px"><img src="' . plugins_url("Refresh-16.png", __FILE__ ) . '" alt=""></a></td>
											...
											<td>
												<table class="dprv" style="padding-right:5px; background-color:#CCCCCC; border:1px solid #666666; border-top:0px; width:796px">
													...
													<tr>
														...
														<td><input name="dprv_content_type" type="text" value="'.htmlspecialchars(stripslashes($dprv_content_type)).'" style="width:290px" onchange="dprv_Preview()" /><span class="description">&nbsp;' . __('e.g. &quot;Blog post&quot;, &quot;Article&quot;, &quot;Opinion&quot;', 'dprv_cp') . '</span></td>
													</tr>
													<tr><td colspan="2" style="height:6px"></td></tr>
													<tr>
														...
														<td><select name="dprv_display_name" style="margin-left:0px;width:300px" onchange="dprv_DisplayNameChanged(this);">
															...
															</select>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:dprv_ShowPrivacyText()">' .__('Note', 'dprv_cp') . '</a>
														</td>
													</tr>
													<tr><td colspan="3" style="height:6px"></td></tr>
													<tr>
										...
										<tr id="ContentPart1" style="display:none">
											<td>
												<table class="dprv" style="padding-right:5px; background-color:#FBBFBF; border:1px solid #666666; border-top:0px; border-bottom:0px; width:798px">
													<tr>
														<td id="dprv_certify_options_caption" colspan="3"><b><em>' . __('When Digiproving Content:', 'dprv_cp') . '</em></b></td>
													</tr>
													<tr><td colspan="2" style="height:6px"></td></tr>
													<tr id="Use_Secure_Protocol"' . $sub_enabled_title . $sub_enabled_onclick . '>
														<td>' . __('Use Secure Protocol:', 'dprv_cp') . '</td>
														<td>
															<input type="checkbox" id="dprv_use_ssl" name="dprv_use_ssl" ' . $dprv_use_ssl_checked . $subscription_enabled_cb . '/>
														</td>
														<td style="padding-left:5px" class="description" ><a href="javascript:dprv_ShowSSLNote()">' .__('More about secure protocol', 'dprv_cp') . '</a></td>
													</tr>
													<tr><td colspan="3" style="height:6px"></td></tr>
										...
										<tr id="ContentPart2" style="display:none">
											<td>
												<table class="dprv" style="padding-right:5px; background-color:#CCFFDD; border:1px solid #666666; border-top:0px; width:798px">
										...
										<tr id="History" style="display:none">
											<td>
												<table class="dprv" style="padding-right:5px; background-color:#AAAAEE; border:1px solid #666666; border-top:0px; width:796px">
													<tr><td style="height:12px; width:260px"></td><td colspan="2"></td></tr>
                                                    <tr><td colspan="3" style="font-size:14px; font-weight:bold">Bulk-Digiprove Older Posts / Pages</td></tr>
													...
													if ($undigiproved_post_count > 0)
													{
														print ('<table>
                                                                    <tr>
                                                                        <td title="' . __("Posts/pages of eligible types not yet Digiproved and not de-selected for Digiproving", "dprv_cp") . '">' . __('Not yet Digiproved: ', 'dprv_cp') . '</td> 
                                                                        <td style="padding-left:10px" id="undigiproved_posts_count">' . $undigiproved_post_count . '</td>
                                                                        <td></td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td>' . __('Remaining bulk-digiprove allowance: ', 'dprv_cp') . '</td> 
                                                                        ...
                                                                        <td style="padding-left:5px" class="description" ><a href="javascript:dprv_ShowBackDigiproveNote()">' .__('Note on bulk-Digiproving', 'dprv_cp') . '</a></td>                                                                        
                                                                    </tr>');
                                                        ...
                                                        {   
                                                            print ('<tr><td style="height:24px"></td></tr>
                                                                    <tr>
                                                                        <td>
                                                                            <button class="button" type="button" id="back_digiprove_now_button" onclick="digiproveAllClicked(' . $undigiproved_post_count . ')">' . __('Bulk-Digiprove Now', 'dprv_cp') . '</button>
                                                                        </td>
                                                                        <td colspan="2">
	...
	$creative_hyperlink = "<a href=\"//www.digiprove.com/creative-and-copyright.aspx\" target=\"_blank\">";
...
*/
	...
	print ('
			...
			dprv_literals["SSL_Note"] = \'' . __("Whether Copyright Proof will communicate with the Digiprove servers using a secure protocol (recommended). The most secure available protocol installed on your server will be chosen.  In order of preference the plugin will look for TLS1.2, TLS1.1, TLS1.0, SSL3 and SSL2.", "dprv_cp") . '\';
			...
			dprv_literals["Default_right_click_alert"] = \'' . __("Sorry, this action is disabled; please respect copyrights", "dprv_cp") . '\';
			...
			dprv_literals["Back_digiprove_note"] = \'' . sprintf(__("You may bulk-Digiprove a number of pre-existing post or pages but it must be done within 30 days of registration. Your allowance depends on your subscription level. Basic (free) users can bulk-digiprove up to 10 posts/pages; for Personal users the limit is 100; above that there is no limit.&nbsp;&nbsp;%sSelect a subscription plan%s.", "dprv_cp"), "<a href=\"" . $dprv_upgrade_link . "&Action=Upgrade\" target=\"_blank\">", "</a>") . '\';
			...
			dprv_literals["Subscribers_only_basic"] = \'' . sprintf(__("The %s function is available only to Digiprove subscribers on a &quot;Personal&quot; or higher plan.  %sSelect a subscription plan%s.", "dprv_cp"), "%1\$s", "<a href=\"" . $dprv_upgrade_link . "&Action=Upgrade\" target=\"_blank\">", "</a>") . '\';
			...
			dprv_literals["Premium_only_personal"] = \'' . sprintf(__("The %s function is available only to Digiprove subscribers at Professional level or above. Your current plan is &quot;%s&quot;.  To use this feature, please %sUpgrade your subscription plan%s.", "dprv_cp"), "%1\$s", $dprv_subscription_type, "<a href=\"" . $dprv_upgrade_link . "&Action=Upgrade\" target=\"_blank\">", "</a>") . '\';
			...
			var dprv_last_result = "' . str_replace("'", "&#39;", str_replace("\r\n", " ", str_replace("\"","&#34", $dprv_last_result))) . '";
	...
	if (isset($_POST['dprv_enrolled']) && $_POST['dprv_enrolled'] == "No" && $_POST['dprv_register'] == "Yes")
	{
		...
		if (isset($_POST['dprv_password']))
		{
			if (isset($_POST['dprv_pw_confirm']) && $_POST['dprv_pw_confirm'] == $_POST['dprv_password'])
			{
				...
				{
					$log->lwrite("returning with password error");
		...
		if (!isset($_POST['dprv_privacy_cb']) || $_POST['dprv_privacy_cb'] !== "Accepted")
		{
			return __('Please accept our Privacy Policy', 'dprv_cp');
		}
	}
	else
	{
...
function dprv_get_log_info()
{
	...
	if ($total_entries == 0)
	{
		return "Event history log is empty";	// Careful, do not change unless changing checks for this value (in js)
	}
...
function dprv_get_log_history($page)
{
	...
	{
	    $log->lwrite("dprv_get_log_history returns false");  
		return false;
	}
...
{
	$log = new DPLog();  
	$log->lwrite("dprv_history_functions starts");
    // The $_REQUEST contains all the data sent via ajax
    if ( isset($_REQUEST) )
	{
		$function = $_REQUEST['function'];
...
function dprv_ajax_functions()
{
	$log = new DPLog();  
	$log->lwrite("dprv_ajax_functions starts");
    // The $_REQUEST contains all the data sent via ajax
    if ( isset($_REQUEST) )
	{
		$function = $_REQUEST['function'];
	    $log->lwrite("dprv_ajax_functions, function = " . $function);

		if ($function == 'dprv_get_last_result')
		{
            $dprv_response = get_option('dprv_last_result');
			$log->lwrite("dprv_response=" . $dprv_response);
			echo json_encode($dprv_response);					// will need to JSON.parse on return to js
		}
...
add_action('wp_ajax_dprv_ajax_functions', 'dprv_ajax_functions');		// initially for get last result
...
{
	$log = new DPLog();  
	$log->lwrite("dprv_get_all_to_be_Digiproved starts");
	$dprv_all = array();
	...
	foreach ($dprv_eligible_post_types as $key => $value)
	{
        ...
        for ($p=0; $p<count($dprv_posts); $p++)
		{
            ...
            if (trim($dprv_post->post_content) == "")
            {
                $log->lwrite("post " . $dprv_post->ID . " content is empty");
                continue;
            }
            $sql = "SELECT * FROM " . get_option('dprv_prefix') . "dprv_posts WHERE id = %d";
            $dprv_rows = dprv_wpdb("get_results", $sql, $dprv_post->ID);
            $dprv_count = count($dprv_rows);
            $dprv_should_be_digiproved = true;
			if ($dprv_count > 0)
			{
                $test_digiprove_this_post = $dprv_rows[0]['digiprove_this_post'];
                $test_certificate_id = "";
                if ($dprv_rows[0]['certificate_id'] != null)
                {
                    $test_certificate_id = $dprv_rows[0]['certificate_id'];
                }
                if ($test_digiprove_this_post == false || trim($test_certificate_id) != "")
                {
                    $dprv_should_be_digiproved = false;
                }
            }
            if ($dprv_should_be_digiproved == true)
            {
                if (!array_search ($dprv_post->ID, $dprv_all))   // if not already lined up
                {
...
{
	$log = new DPLog();
    $log->lwrite("starting dprv_certify_post");
	$digital_fingerprint = "";
    ...
    else
    {
        ...
        update_option('dprv_event','');  // if we got here, there is an intelligible response we can assume that any dprv_event has been reported, it can be cleared
